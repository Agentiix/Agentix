...
async def test_concurrent_remote_calls_share_worker(live_server):
    ...
    try:
        async with RuntimeClient(base_url) as c:
            ...
            results = await asyncio.gather(*[
                c.remote(emit_and_sleep, label=f"r{i}", duration=duration)
                for i in range(n)
            ])
    ...
    assert len(collected_ends) == n, (
        f"expected {n} concurrent_test span_end events, got {len(collected_ends)}"
    )
    ...
    assert len(call_ids) == n, (
        f"expected {n} distinct call_ids, got {len(call_ids)}: {call_ids}"
    )
