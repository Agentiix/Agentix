---
title: Development
description: Local setup, daily workflow, and how to add a new closure.
---
...
- [Docker](https://docs.docker.com/get-docker/) — the only host
  ...
  `tests/closure-docker/Dockerfile`.
...
Without any closures mounted, the server exposes its built-ins
(`/health`, `/exec`, `/upload`, `/download`). Useful for iterating on
the runtime without the Docker round-trip.
...
Everything inside the sandbox is a closure. Copy a reference and write
your own Dockerfile:
...
See [Namespace protocol](/reference/namespace-protocol) for the closure
ABI.
